...
statement: expression_stmt {$<node>$ = newSyntaxTreeNode("statement");SyntaxTreeNode_AddChild($<node>$, $<node>1);callback(1, $<node>$);}
         ...
         | iteration_stmt {$<node>$ = newSyntaxTreeNode("statement");SyntaxTreeNode_AddChild($<node>$, $<node>1);callback(1, $<node>$);}
...
SyntaxTree* syntax(const char* input)
{
	gt = newSyntaxTree();
	...
	/*if (!fp) {*/
		/*fprintf(stderr, "[ERR] Open output file %s failed.\n", output);*/
	/*}*/
	...
	if ( yyparse() != 0 ) {
		fprintf(stderr, "[ERR] Error parsing %s. Abort. \n", input);
		return NULL;
	}
	else {
...
/*#endif*/
	...
	/*}*/
	fprintf(stderr, "Syntax test not implemented.\n");
	return 0;
}
